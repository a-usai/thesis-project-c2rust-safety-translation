...
// ──────────────────────────────────────────────────────────────────────────────
// MinHeap è rappresentato come una struct che possiede un Vec<MinHeapNode>.
...
impl MinHeapNode {
    ...
    pub fn new(data: u8, freq: u32) -> Self {
        MinHeapNode {
            data,
            freq,
            left:  None,
...
/// Min-Heap di nodi Huffman, ownership esclusiva tramite Vec<MinHeapNode>.
pub struct MinHeap {
    ...
    /// Vec<MinHeapNode> (senza Box) evita la doppia indirezione: i nodi
    /// risiedono direttamente nel buffer contiguo del vettore (cache-friendly).
    array: Vec<MinHeapNode>,
}
...
impl MinHeap {
    ...
    pub fn extract_min(&mut self) -> Option<MinHeapNode> {
        if self.size == 0 {
            return None;
        }
    ...
    ///
    /// Complessità: O(log n).
    pub fn insert(&mut self, node: MinHeapNode) {
...
pub fn build_huffman_tree(data: &[u8], freq: &[u32]) -> MinHeapNode {
    assert_eq!(data.len(), freq.len(), "data e freq devono avere la stessa lunghezza");
    let n = data.len();

    let mut heap = MinHeap::with_capacity(n);
    for i in 0..n {
        // MinHeapNode::new restituisce MinHeapNode (non Box): nessuna doppia indirezione.
    ...
    while !heap.is_size_one() {
        ...
        // I figli vengono spostati in Box perché left/right sono ricorsivi
        // e richiedono dimensione nota al compilatore.
        let combined_freq = left.freq + right.freq;
        let internal = MinHeapNode {
            data:  b'$',
            freq:  combined_freq,
